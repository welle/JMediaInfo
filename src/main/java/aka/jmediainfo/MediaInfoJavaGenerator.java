...
public final class MediaInfoJavaGenerator {
    ...
    public static void main(final String[] args) {
        ...
        String path2;
        String basePackage;
        String basePackageTest;
        if (args == null || args.length < 4) {
            path = "C:/Projects/Own/JMediaInfo/generatedSRC/main/";
            path2 = "C:/Projects/Own/JMediaInfo/generatedSRC/test/";
            basePackage = "aka.jmetadata.main";
            basePackageTest = "aka.jmetadata.test";
        } else {
            path = args[0];
            path2 = args[1];
            basePackage = args[3];
            basePackageTest = args[3];
        }

        if (path == null || path2 == null || basePackage == null || basePackageTest == null) {
            LOGGER.logp(Level.SEVERE, "MediaInfoJavaGenerator", "main", "No path");
        } else {
            ...
            for (final Entry<@NonNull String, String> entry : KINDSSTREAM.entrySet()) {
                assert entry != null;
                constantsCreator.parseStreamKind(entry, path, path2, basePackage, basePackageTest);
            }
            for (final Entry<@NonNull String, String> entry : KINDS.entrySet()) {
                assert entry != null;
                constantsCreator.parseKind(entry, path, path2, basePackage, basePackageTest);
            }
        }
    }

    private boolean parseStreamKind(@NonNull final Entry<@NonNull String, String> entry, @NonNull final String path, @NonNull final String path2, @NonNull final String basePackage, @NonNull final String basePackageTest) {
        ...
        createFileStream(entry.getKey(), map, path, basePackage);

        createFileTest(entry.getKey(), map, path2, basePackage, basePackageTest);

        return success;
    }

    private boolean parseKind(@NonNull final Entry<@NonNull String, String> entry, @NonNull final String path, @NonNull final String path2, @NonNull final String basePackage, @NonNull final String basePackageTest) {
        final boolean success = false;
        ...
        createFile(entry.getKey(), map, path, basePackage);
    ...
    private void createConstantsFile(@NonNull final String kind, @NonNull final Map<@NonNull String, String> map, @NonNull final String path, @NonNull final String basePackage) {
        try {
            final Path file = Paths.get(path + "constants/" + kind + ".java");

            final List<@NonNull String> javaLines = new ArrayList<>();

            javaLines.add("package " + basePackage + ".constants;");
            javaLines.add("");
            ...
            LOGGER.logp(Level.SEVERE, "MediaInfoJavaGenerator", "createConstantsFile", e.getMessage(), e);
        }
    }

    private void createFile(@NonNull final String kind, @NonNull final Map<@NonNull String, String> map, @NonNull final String path, @NonNull final String basePackage) {
        try {
            final Path file = Paths.get(path + "JMetaData" + kind + ".java");

            final List<@NonNull String> javaLines = new ArrayList<>();

            javaLines.add("package " + basePackage + ";");
            javaLines.add("");
            ...
            javaLines.add("");
            javaLines.add("import " + basePackage + ".constants." + kind + ";");
            javaLines.add("import " + basePackage + ".constants.StreamKind;");
            javaLines.add("import " + basePackage + ".mediainfo.MediaInfo;");
            ...
            LOGGER.logp(Level.SEVERE, "MediaInfoJavaGenerator", "createFile", e.getMessage(), e);
        }
    }

    private void createFileStream(@NonNull final String kind, @NonNull final Map<@NonNull String, String> map, @NonNull final String path, @NonNull final String basePackage) {
        try {
            final Path file = Paths.get(path + "JMetaData" + kind + ".java");
    ...
    private void createFileTest(@NonNull final String kind, @NonNull final Map<@NonNull String, String> map, @NonNull final String path, @NonNull final String basePackage, @NonNull final String basePackageTest) {
        try {
            final Path file = Paths.get(path + "JMetaData" + kind + "_Test.java");

            final List<@NonNull String> javaLines = new ArrayList<>();

            javaLines.add("package " + basePackageTest + ";");
            javaLines.add("");
            ...
            javaLines.add("");
            javaLines.add("import " + basePackage + ".JMetaData;");
            javaLines.add("import " + basePackage + ".JMetaData" + kind + ";");
            javaLines.add("");
            javaLines.add("/**");
